...
import sys
import fileinput
# from g2p_dependency._Mecab import Mecab
from .g2p_mecab import Mecab
...
def vocab2pronun(match):
    ...
    #print("Text : ", tline)
    pStr = toPhonemeString(tline)
    #print("text -> Phoneme : " , pStr)
    pronun = pString2Pronun(pStr)
    #print("\tPhoneme -> Pronun: " , pStr)

    # seperate pronunciation into each morpheme
    hPronun = toHangul(pronun)
    #print("\tPronun -> hangul: " , hPronun)

    tline = re.sub('[ ]', '', line_)
    morphs = tline.split('+')
    #print("text -> morpheme", morphs)
    idx = 0
    tList = []
    tList2 = []
    for morph in morphs:
        ...
        tList.append(toPhonemeString(morph))
        tList2.append(toPhonemeString(morph_p))
    #print("\tmophme -> tlist", tList)
    #print("\tmophme -> tlist2", tList2)
    ...
    tStr2 = " + ".join(tList2)
    #print("\ttlist -> result", tStr2)
    return pronun2psymbol(tStr2)



def convertSentence2Pronounce(sentence):

    # remove invalid symbol
    line = re.sub(r'[^가-힣 .,!\'-?~]', '', sentence)

    # # remove duplicated symbol
    # line = re.sub(r'([ .!\'-,?])\1+', r'\1', line).strip()

    # substitue ' ' to '@'
    line = re.sub(r'[" "]', '@', line)

    # add empty space between the punctuation
    line = re.sub(r'([^가-힣])', r' \1 ', line)

    # remove useless space
    line = re.sub(r'([ .!\'-,?])\1+', r'\1', line).strip()

    return re.sub(r'([가-힣]+)', vocab2pronun, line)

if __name__ == '__main__':
    print(convertSentence2Pronounce("앙팡의 잉잉차 핳핳햐 하얗게 익현이가 그들의 과자를 모두 먹어버렸다."))
